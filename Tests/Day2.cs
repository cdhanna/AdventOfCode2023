...
public class Day2
{
    ...
    [TestCase(SampleInput, 8)]
    [TestCase(Part1Input, 2156)]
    public void Part1(string input, int expected)
    {
        ...
        foreach (var game in games)
        {
            ...
            var gameNumber = int.Parse(parts[0].Replace("Game ", ""));

            var valid = true;

            var rounds = parts[1].Split(';', StringSplitOptions.RemoveEmptyEntries);
            foreach (var round in rounds)
            {
                var groups = round.Split(',', StringSplitOptions.RemoveEmptyEntries);
                foreach (var group in groups)
                {
                    var info = group.Split(' ', StringSplitOptions.RemoveEmptyEntries);
                    var count = int.Parse(info[0]);
                    var color = info[1];

                    if (!limits.TryGetValue(color, out var limit) )
                    {
    ...
    [TestCase(SampleInput, 2286)]
    [TestCase(Part1Input, 66909)]
        ...
        foreach (var game in games)
        {
            ...
            foreach (var round in rounds)
            {
                ...
                foreach (var group in groups)
                {
                    ...
                    if (!maxes.TryGetValue(color, out var oldMax) )
                    {
                        throw new Exception("invalid color");
                    }
    ...
    #region inputs
    private const string SampleInput = @"Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green";

    private const string Part1Input = @"Game 1: 4 blue, 4 red, 16 green; 14 green, 5 red; 1 blue, 3 red, 5 green
