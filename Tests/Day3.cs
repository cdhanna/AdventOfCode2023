...
public class Day3
{
    [TestCase(SampleInput, 4361)]
    [TestCase(Input, 556057)]
    public void Part1(string input, int expected)
    {
        var lines = input.Split(Environment.NewLine);
        var coords = new Dictionary<Vector2, char>();
        var sum = 0;
        var coordsToCheck = new List<Vector2>();
        ...
        {
            var neighbors = coord.GetNeighborsDiag();
            var used = new HashSet<Vector2>();
            foreach (var neighbor in neighbors)
            {
                if (neighbor.X < 0 || neighbor.Y < 0 || neighbor.X >= lines[0].Length || neighbor.Y >= lines.Length)
                    continue;

                if (used.Contains(neighbor)) continue;
                
                var c = coords[neighbor];
                if (char.IsDigit(c))
                {
                    used.Add(neighbor);
                    var line = lines[(int)neighbor.Y];
                    var startX = (int)neighbor.X;
                    ...
                    for (var x = startX - 1; x >= 0; x--)
                    {
                        ...
                        {
                            minX = x;
                            used.Add(new Vector2(x, neighbor.Y));
                        }
                        else
                        {
                            break;
                    ...
                    // expand right
                    for (var x = startX + 1; x < line.Length; x++)
                    {
                        if (char.IsDigit(line[x]))
                        {
                            maxX = x;
                    ...
                    var number = line.Substring(minX, (maxX - minX) + 1);
    ...
    [TestCase(SampleInput, 467835)]
    [TestCase(Input, 82824352)]
    ...
    public const string SampleInput = @"467..114..
...*......
..35..633.
......#...
617*......
